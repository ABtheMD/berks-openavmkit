...
def build_settings(
    ...
    settings: dict = {
        "__generator": (
            ...
            "Run configure_settings.py next to fill load maps and model_groups."
        ),
        "locality": locality,
        "data": {
            "load": data_load,
        ...
        "modeling": {
            ...
            "models": {
                "main": {
                    "default": {
                        "__ind_vars_note": (
                            "Independent variables (predictors) for the model. "
                            "When empty, falls back to try_variables.variables."
                        ),
                        "ind_vars": [],
            ...
            "__model_groups_note": (
                ...
                "'filter' is a filter expression using openavmkit filter syntax. "
                "String literals in filters must use the 'str:' prefix. "
                "Example: {\"res\": {\"name\": \"Residential\", \"filter\": [\"==\", \"class\", \"str:R\"]}}"
            ),
            "model_groups": {},
        },
        "analysis": {
...
""",
    ...
    print(f"\n[ok] Seed loaded       : {locality_name}", file=sys.stderr)
    ...
    print(f"[ok] Data dictionary   : {len(data_dict)} canonical fields", file=sys.stderr)
    ...
    print(f"\n[ok] Fields processed  : {total}", file=sys.stderr)
    ...
    settings = build_settings(seed, classification)
    ...
    print(
        "\n  Next steps:\n"
        "    1. Run  python scripts/download_data.py <seed>  to download the parquet data\n"
        "    2. Run  python scripts/configure_settings.py <seed>  to auto-fill\n"
        "       load maps, model_groups, and important fields from the downloaded data\n"
        "    3. Review the final settings.json — especially modeling.metadata\n"
        "       (modeler, modeler_nick, valuation_date are pre-filled)\n",
        file=sys.stderr,
    )
